...
def augment_signals(input_data):
    output_rotperm=np.empty(input_data.shape, dtype=float)
    output_rotation=np.empty(input_data.shape, dtype=float)
    output_permutation=np.empty(input_data.shape, dtype=float)

    for i in range(input_data.shape[0]):
        X=input_data[i,:,:]
        output_rotation[i,:,:]=DA_Rotation(X)
        output_permutation[i,:,:]=DA_Permutation(X, nPerm=4, minSegLength=10)
        output_rotperm[i,:,:]=DA_Rotation(DA_Permutation(X, nPerm=2))
       
    return(output_rotation,output_permutation,output_rotperm)
